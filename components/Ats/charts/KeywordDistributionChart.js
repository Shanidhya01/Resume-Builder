...
const COLORS = ['#3b82f6', '#60a5fa', '#34d399', '#fbbf24', '#f87171', '#f472b6', '#38bdf8'];

const KeywordDistributionChart = ({ keywords = [] }) => {
    const data = keywords.slice(0, 10).map(k => ({ keyword: k.keyword, count: k.count }));

    if (data.length === 0) {
        return <p className="text-sm text-fg-muted">No keyword data yet — add more content to your resume.</p>;
    }
