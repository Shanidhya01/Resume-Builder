...
/**
 * GoogleButton — OAuth entry point styled with design-system tokens.
 * Logic (onClick) is owned by the auth pages; this is presentation only.
 */
const GoogleButton = ({ onClick, disabled, loading, label = 'Continue with Google' }) => (
    <button
        type="button"
        onClick={onClick}
        disabled={disabled}
        className="flex h-11 w-full items-center justify-center gap-3 rounded-xl border border-line-strong bg-surface px-3 text-sm font-semibold text-fg outline-none transition-colors hover:bg-surface-2 focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-canvas disabled:cursor-not-allowed disabled:opacity-60"
    >
        {loading ? (
