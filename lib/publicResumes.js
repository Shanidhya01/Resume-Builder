...
// NOTE: This module is imported by CLIENT components (Share dialog/panel,
// analytics panel) and therefore uses the Firebase *client* SDK, authenticated
// as the resume owner. All owner-only mutations below rely on Firestore
// security rules for authorization. Server-side, unauthenticated public
// operations (resolve-by-slug + analytics increments) live in
// lib/publicResumes.server.js and use the Admin SDK instead.
...
// Public-facing analytics WRITES (view/download/share counters) were moved to
// lib/publicResumes.server.js and now run via the Admin SDK from the rate-limited
// API routes — anonymous visitors no longer write to Firestore directly.
