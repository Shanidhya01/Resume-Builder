...
export const duplicateResume = async (id, uid) => {
    ...
    // Strip identity, timestamps AND all public-sharing/analytics state. A copy
    // must NOT inherit the original's slug/isPublic: the slug in /publicSlugs
    // still maps to the ORIGINAL document, so a duplicate that kept it would
    // render as "Public" with a link that silently resolves to a different
    // resume. Copies always start private with a clean analytics slate.
    const {
        id: _omit,
        createdAt,
        updatedAt,
